...
func (s *HttpServer) StartServer() error {
	...
	router.Path("/tasks/{title}").Methods("PATCH").HandlerFunc(middleware.AuthMiddleware(s.httpHandlers.HandleCompleteTask))
	router.Path("/tasks/{title}").Methods("DELETE").HandlerFunc(middleware.AuthMiddleware(s.httpHandlers.HandleDeleteTask))
